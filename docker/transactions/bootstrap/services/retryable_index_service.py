...
class IndexService:
    ...
    def reindex_master_copies(
        ...
        if not addresses:
            ...
            # Original source code here: https://github.com/gnosis/safe-transaction-service/blob/v3.4.17/safe_transaction_service/history/services/index_service.py
            # They only difference is retry logic because Alchemy will sometimes randomly return the following response:
            # {'jsonrpc': '2.0', 'id': 0, 'error': {'code': -32601, 'message': 'the method trace_transaction does not exist/is not available'}} and safe-transaction-service
            # doesn't currently handle that gracefully
            retry_count = 0
            while block_number < stop_block_number:
                try:
                    ...
                    if retry_count > self.MAX_RETRY_ATTEMPTS:
                        ...
                        logger.warning(
                            "HTTPError while processing elements. Retrying %s", retry_count
                        )
                        time.sleep(8)
                        retry_count += 1
                        continue
                try:
                    ...
                    if retry_count > self.MAX_RETRY_ATTEMPTS:
                        ...
                        logger.warning(
                            "KeyError while processing elements. Retrying %s", retry_count
                        )
                        time.sleep(8)
                        retry_count += 1
                        continue
